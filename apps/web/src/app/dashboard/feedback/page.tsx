...
import { Plus, Search, ChevronDown, ChevronUp, MoreHorizontal, Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import { ApiError, EmptyState } from '@/components/api-error';

type FeedbackStatus = 'under_review' | 'planned' | 'in_progress' | 'shipped' | 'wont_do';
...
export default function FeedbackPage() {
  ...
  const createMutation = trpc.feedback.create.useMutation({
    onSuccess: () => {
      ...
      setNewItem({ title: '', description: '', category: 'feature', status: 'under_review' });
  ...
  if (error) {
    return <ApiError error={error} onRetry={() => refetch()} title="Failed to load feedback" />;
  }
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="sm:max-w-[400px]">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="grid gap-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewItem({ ...newItem, status: value as FeedbackStatus })}
                  >
                    <SelectTrigger>
            ...
            <DialogFooter className="pt-4">
              ...
              <Button size="sm" onClick={handleCreate} disabled={!newItem.title.trim() || createMutation.isPending}>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => setSortBy('votes')}>
                Top Voted
                {sortBy === 'votes' && <span className="ml-auto text-foreground">✓</span>}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSortBy('createdAt')}>
                Most Recent
                {sortBy === 'createdAt' && <span className="ml-auto text-foreground">✓</span>}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        {filteredFeedback.length === 0 ? (
          <EmptyState
            ...
            description={search ? 'Try adjusting your search' : 'Create your first feedback item to get started'}
            action={!search ? { label: 'Add Feedback', onClick: () => setIsCreateOpen(true) } : undefined}
          />
        ) : (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  >
                    ...
                    <span className="text-xs font-medium leading-none">{item.voteCount}</span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-xs text-muted-foreground capitalize">
                        {categoryLabels[item.category ?? 'other'] || item.category || 'Other'}
                      </span>
                  ...
                  {/* Status Badge */}
                  {showStatus && (
                    <span className={cn('shrink-0 text-[11px] leading-none px-1.5 py-1 rounded', statusColors[item.status])}>
