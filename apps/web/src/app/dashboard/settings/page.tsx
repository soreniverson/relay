  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <div className="w-56 flex-shrink-0 border-r border-border py-2" style={{ borderRightWidth: '0.5px' }}>
...
function GeneralSettings() {
  return (
    <div className="space-y-6">
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-1.5">
            <Label htmlFor="project-url" className="text-xs text-muted-foreground">Project URL</Label>
      ...
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        <h3 className="text-sm font-medium text-foreground mb-3">Data Region</h3>
        <p className="text-xs text-muted-foreground mb-3">
          Your data is stored in the following region. This cannot be changed after project creation.
        </p>
        <div className="flex items-center gap-2 p-2.5 bg-muted/50 rounded-md border border-border" style={{ borderWidth: '0.5px' }}>
          <span className="text-lg">🇺🇸</span>
          <div>
            <div className="text-sm font-medium text-foreground">US West (Oregon)</div>
            <div className="text-xs text-muted-foreground">us-west-2</div>
          </div>
        </div>
      </div>

      <div className="border border-red-500/20 rounded-md p-4 bg-red-500/5" style={{ borderWidth: '0.5px' }}>
      ...
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        ...
        <div className="space-y-1 text-sm">
          {[
            ...
            <div key={item.role} className="flex justify-between p-2 rounded-md bg-muted/30">
...
function ApiKeysSettings() {
  ...
  const [newKeyScopes, setNewKeyScopes] = useState<('ingest' | 'read' | 'write' | 'admin')[]>(['ingest', 'read']);
  ...
  if (!currentProject) {
    return <div className="text-muted-foreground text-sm">Select a project to manage API keys</div>;
  }
    ...
    <div className="space-y-6">
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        <div className="flex items-center justify-between mb-4">
          ...
          <Button size="sm" onClick={() => setShowCreateDialog(true)}>Create Key</Button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {keys.map((key) => (
              <div key={key.id} className="p-3 bg-muted/30 rounded-md border border-border" style={{ borderWidth: '0.5px' }}>
      ...
      <div className="border border-amber-500/20 rounded-md p-3 bg-amber-500/5" style={{ borderWidth: '0.5px' }}>
        <h4 className="text-xs font-medium text-amber-400 mb-1">Security Note</h4>
        <p className="text-xs text-amber-400/70">
          API keys grant access to your project data. Never expose them in client-side code or public repositories.
        </p>
      </div>
...
function IntegrationsSettings() {
  const integrations = [
    { name: 'Linear', description: 'Create Linear issues from bug reports', connected: true, icon: '📋' },
    { name: 'Slack', description: 'Get notifications in Slack', connected: true, icon: '💬' },
    { name: 'Jira', description: 'Create Jira tickets from bug reports', connected: false, icon: '🎫' },
    { name: 'GitHub', description: 'Create GitHub issues from bug reports', connected: false, icon: '🐙' },
  ];

  return (
    <div className="space-y-6">
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        <h3 className="text-sm font-medium text-foreground mb-4">Integrations</h3>
        <div className="space-y-1">
...
function PrivacySettings() {
  return (
    <div className="space-y-6">
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        <h3 className="text-sm font-medium text-foreground mb-4">Privacy Rules</h3>
        <p className="text-xs text-muted-foreground mb-4">
          Configure what data is captured and masked in bug reports and session replays.
        </p>

        <div className="space-y-2">
          {[
            { id: 'mask-inputs', label: 'Mask all input fields', desc: 'Automatically mask input field values in replays', defaultChecked: true },
            { id: 'mask-emails', label: 'Mask email addresses', desc: 'Detect and mask email addresses in captured data', defaultChecked: true },
            { id: 'block-network', label: 'Block network request bodies', desc: "Don't capture request/response bodies in network logs", defaultChecked: false },
          ].map((item) => (
            <div key={item.id} className="flex items-center justify-between p-2.5 rounded-md bg-muted/30">
              <div>
      ...
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        <h3 className="text-sm font-medium text-foreground mb-3">Custom Selectors</h3>
        <p className="text-xs text-muted-foreground mb-3">
          Add CSS selectors to mask or block specific elements.
        </p>
        <div className="space-y-3">
          <div className="grid gap-1.5">
            <Label className="text-xs text-muted-foreground">Mask selectors</Label>
            <Input placeholder=".sensitive-data, [data-private]" className="font-mono text-sm" />
          </div>
          <div className="grid gap-1.5">
            <Label className="text-xs text-muted-foreground">Block selectors</Label>
            <Input placeholder=".credit-card, .ssn" className="font-mono text-sm" />
          </div>
        </div>
        <Button size="sm" className="mt-4">Save Rules</Button>
      </div>
      ...
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        ...
        <div className="space-y-2">
          {[
            { label: 'Bug reports & feedback', options: ['365 days', '180 days', '90 days', '30 days'], default: '365 days' },
            { label: 'Session replays', options: ['30 days', '14 days', '7 days'], default: '30 days' },
            { label: 'Audit logs', options: ['2 years', '1 year'], default: '2 years' },
          ].map((item) => (
        ...
        <Button size="sm" className="mt-4">Save Retention Settings</Button>
      </div>
    </div>
  );
}
...
function BillingSettings() {
  return (
    <div className="space-y-6">
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        <h3 className="text-sm font-medium text-foreground mb-3">Current Plan</h3>
      ...
      <div className="border border-border rounded-md p-4" style={{ borderWidth: '0.5px' }}>
        <h3 className="text-sm font-medium text-foreground mb-3">Billing History</h3>
