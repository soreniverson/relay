...
export default function WorkflowsPage() {
  ...
  const filteredWorkflows = workflows.filter((workflow) => {
    ...
    if (search && !workflow.name.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
  ...
  const handleCreate = () => {
    ...
    setNewWorkflow({ name: '', description: '', trigger: 'interaction.created', action: 'send_slack' });
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className="text-xs text-muted-foreground/70">
                      {workflow.actions.map(a => actionLabels[a] || a).join(', ')}
                    </span>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="sm:max-w-[400px]">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button size="sm" onClick={handleCreate} disabled={!newWorkflow.name.trim()}>
              Create
            </Button>
          </DialogFooter>
        </DialogContent>
