...
export default function RoadmapPage() {
  ...
  if (error) {
    return <ApiError error={error} onRetry={() => refetch()} title="Failed to load roadmap" />;
  }
  ...
  return (
    <div className="flex flex-col h-full">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="grid gap-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setNewItem({ ...newItem, status: value as RoadmapStatus })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="grid gap-1.5">
                    <Label htmlFor="eta" className="text-xs text-muted-foreground">ETA</Label>
                    <Select
                      value={newItem.eta}
                      onValueChange={(value) => setNewItem({ ...newItem, eta: value })}
                    >
                      <SelectTrigger>
      ...
      {/* Column Headers */}
      <div className="flex items-center h-[41px] border-b border-border" style={{ borderBottomWidth: '0.5px' }}>
        {columns.map((column, index) => (
          <div
            ...
            style={index < columns.length - 1 ? { borderRightWidth: '0.5px' } : undefined}
          >
      ...
      <div className="flex-1 flex min-h-0">
        {columns.map((column, index) => {
          ...
          const isDifferentColumn = draggedItemData && draggedItemData.status !== column.key;
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isDragging && 'opacity-40 scale-[0.98] shadow-lg rotate-1'
                          )}
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between text-xs text-muted-foreground">
                            ...
                            {item.eta && <span>{new Date(item.eta).toLocaleDateString('en-US', { year: 'numeric', month: 'short' })}</span>}
                          </div>
                        </div>
                      );
                    })}
                </div>
                {items.filter((item) => item.status === column.key).length === 0 && (
