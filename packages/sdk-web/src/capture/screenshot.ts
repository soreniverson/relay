...
function maskElements(doc: Document, selectors: string[]): (() => void) {
  ...
  selectors.forEach((selector) => {
    doc.querySelectorAll<HTMLElement>(selector).forEach((el) => {
      ...
      el.innerHTML = '<span style="background:#ccc;display:block;width:100%;height:100%"></span>';
    });
  });
...
async function captureWithCanvas(options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
  const {
    quality = 0.92,
    format = 'png',
    maxWidth = 1920,
      ...
      annotations.forEach((annotation) => {
        ...
        switch (annotation.type) {
          ...
          case 'arrow':
            ...
            ctx.stroke();
            // Arrowhead
            const angle = Math.atan2(annotation.height || 0, annotation.width || 0);
            const headLength = 15;
            ctx.beginPath();
          ...
          case 'highlight':
            ...
            ctx.fillRect(annotation.x, annotation.y, annotation.width || 100, annotation.height || 30);
            break;
